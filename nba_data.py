...
def run(date_end_str, boxscores, links_games, data_players, simulation=False):
    days_samplesize = 30
    outcomes = np.array()
...
#     get outcomes if simulation is true
    if simulation:
        outcomes = get_scores(links_games)
    
#     predict spread for all boxscores
    predict_spreads_games(boxscores, data_players)
...
def get_boxscores(links_games, simulation=False):
    boxscores = []
    outcomes = []
     
    for game in links_games:
        if simulation:
            boxscore, outcome = get_boxscore(game, simulation=True)
...
def get_boxscore(link_game, simulation=False):
...
def extract_player_data(boxscores, dataset):
    for box in boxscores:
            ...
            if is_valid_player:                
                ...
                if player_secondsplayed > 0:
                    ...
                    if player_id in dataset:
                        dataset[player_id].data = dataset[player_id].data + player_data_arr
                    else:
                        dataset[player_id] = Player(player_name, player_data_arr)


def predict_spreads_games(boxscores, data_players):
    print ("simulation starting: %s" % is_simulation)
    predictions = []
